...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            args.logger.info("=> loading checkpoint '{}'".format(args.pretrained))
            ...
            for k in list(state_dict.keys()):
                ...
                for source, target in zip(
                    ...
                    if k.startswith(source):
                        state_dict[target + k[len(source):]] = state_dict[k]
                        del state_dict[k]

            args.start_epoch = 0
            msg = model.load_state_dict(state_dict, strict=False)
            args.logger.info("Missing keys: ")
            print(msg.missing_keys)
            assert set(msg.missing_keys) == {"fc.weight", "fc.bias"}
            args.logger.info("=> loaded pre-trained model '{}'".format(args.pretrained))
        else:
            args.logger.info("=> no checkpoint found at '{}'".format(args.pretrained))
